...
if [ ! -f ${cbfstoolcmd} ]; then
    ...
    echo_yellow "Downloading cbfstool utility (from local)"
    $CURL -sLO "${local_util_source}cbfstool.tar.gz"
    if [ $? -ne 0 ]; then
...
if [ ! -f ${flashromcmd} ]; then
    ...
    echo_yellow "Downloading flashrom utility (from local)"
    if [[ "$isChromeOS" = true ]]; then
        #needed to avoid dependencies not found on older ChromeOS
        $CURL -sLo "flashrom.tar.gz" "${local_util_source}flashrom_old.tar.gz"
    else
        $CURL -sLO "${local_util_source}flashrom.tar.gz"
    fi
    if [ $? -ne 0 ]; then
...
if [ ! -f ${gbbutilitycmd} ]; then
    ...
    echo_yellow "Downloading gbb utility (from local)"
    $CURL -sLO "${local_util_source}gbb_utility.tar.gz"
    if [ $? -ne 0 ]; then
