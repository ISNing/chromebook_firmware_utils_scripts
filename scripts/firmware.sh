...
if [[ ${rwlegacy_file} = "seabios-kbl-mrchromebox_20200223.bin" ]]; then
    echo_yellow "\nFrom local"
    curl -s -L -O "${local_rwlegacy_source}${rwlegacy_file}.md5"
    curl -s -L -O "${local_rwlegacy_source}${rwlegacy_file}"
else
    echo_yellow "\nFrom mrchromebox's site"
...
if [ "$preferUSB" = true  ]; then
    ...
    if [[ "$device" = "swanky" ]]; then
	 echo_yellow "\nDownloading bootorder file from mrchromebox"
    	$CURL -sLo bootorder "${cbfs_source}bootorder.usb2"
    else
	 echo_yellow "\nDownloading bootorder file from local"
		$CURL -sLo bootorder "${local_cbfs_source}bootorder.usb"
    fi
    if [ $? -ne 0 ]; then
